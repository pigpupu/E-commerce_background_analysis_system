<template>
  ...
  <el-row justify="center" style="top: 100px">
    <el-col :span="8" style="margin-right : 110px">
      <single-bar-chart :horizontal="backData.names" :vertical="backData.values" :barTitleText="barTitleText"></single-bar-chart>
    </el-col>
    <el-col :span="8">
      <single-line-chart :horizontal="backData.names" :vertical="backData.values" :lineTitleText="lineTitleText"></single-line-chart>
    </el-col>
    <el-col :span="8" style="margin-left : 20px ; margin-top: 100px;">
      <basic-pie-chart :classify="backData.names" :piedata="backData.pieMapVoList" :pieTitleText="'销售额饼图'"></basic-pie-chart>
    </el-col>
  </el-row>

  <el-row justify="center" style="top: 200px">
    <el-col :span="24">
      <basic-map-chart :mapdata="backMapData.pieMapVoList" :titleText="titleText"></basic-map-chart>
    </el-col>
  </el-row>
  ...
  <el-row justify="center" style="top: 300px">
    ...
    <div>该分类下的TOP销售额商品</div>
    <el-col style="margin-bottom: 80px;">
      <!--<Table :data="tableData2" :column="tableTitle2" :operation="false" :page="page" :loading="loading"
      @setCellColor="setCellColor" style="margin-top : 100px">
      </Table>-->
...
import {
  queryAllCategory, basicMoneyChart, basicMoneyPage,
  Top10MoneyGoodVo, basicMoneyMapChart
} from "../../api/systemApi.js"

const state = reactive({
  barTitleText:"销售额柱状图",
  titleText:"中国区域销售额情况分析图",
  lineTitleText:"销售额条形图",
  searchForm: {
    startTimeStr: null,
    endTimeStr: null,
    categoryID: null,
  ...
  tableTitle: [
    ...
    { prop: 'periodMoneyTotal', label: '销售额' },
  ],
  tableTitle2: [
    { prop: '', label: '', fixed: 'left' },
...
const { barTitleText,lineTitleText,titleText,searchForm, page, page2,timeRange, catOptions, tableData, tableData2, tableTitle, tableTitle2, backData, backMapData } = toRefs(state);
...
let onSubmit = () => {
  ...
  basicMoneyChart(state.searchForm).then((response) => {
  ...
  basicMoneyPage(Object.assign(state.page, state.searchForm)).then((response) => {
    if (response.data.code == 200) {
  ...
  basicMoneyMapChart(state.searchForm).then((response) => {
    if (response.data.code == 200) {
